...
import mpqp.core.instruction.gates.native_gates
...
def test_all_native_gates():
    circuit = QCircuit(4, label="Test noisy native gates")
    circuit.add([H(0), X(1), Y(2), Z(0), S(1), T(0)])
    circuit.add([Rx(1.2324, 2), Ry(-2.43, 0), Rz(1.04, 1), Rk(-1, 3), P(-323, 2)])
